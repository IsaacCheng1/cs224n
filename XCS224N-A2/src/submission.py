...
def naive_softmax_loss_and_gradient(center_word_vec,outside_word_idx,outside_vectors,dataset):
  ...
  ### START CODE HERE
  # compute loss between current outside word and center word
  probs = softmax(np.dot(outside_vectors, center_word_vec.T))
  prob = probs[outside_word_idx]
  loss = -np.log(prob)

  # compute the gradient of the loss function with respect to the center word vector
  y_hat = probs
  y_hat = y_hat.reshape((y_hat.size, -1))
  y = np.zeros(y_hat.shape)
  y[outside_word_idx] = 1
  grad_center_vec = np.dot((y_hat - y).T, outside_vectors)

  # compute the gradient of the loss function with respetc to the outside word vector
  grad_outside_vecs = np.dot(y_hat - y, center_word_vec.reshape((-1, center_word_vec.size)))
  ### END CODE HERE
...
def skipgram(current_center_word, window_size, outside_words, word2ind, center_word_vectors, outside_vectors, dataset, word2vec_loss_and_gradient=neg_sampling_loss_and_gradient):
  ...
  ### START CODE HERE
  center_word_vec_idx = word2ind[current_center_word]
  center_word_vec = center_word_vectors[center_word_vec_idx]

  for outside_word in outside_words:
    outside_word_idx = word2ind[outside_word]
    l, grad_center_vec, grad_outside_vecs = word2vec_loss_and_gradient(center_word_vec, outside_word_idx, outside_vectors, dataset)
    loss += l
    grad_center_vecs[center_word_vec_idx:center_word_vec_idx+1] += grad_center_vec
